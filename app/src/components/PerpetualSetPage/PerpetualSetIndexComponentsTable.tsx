...
import { OPTIMISM_CHAIN_DATA, POLYGON_CHAIN_DATA } from 'utils/connectors'
...
const ComponentRow: React.FC<ComponentRowProps> = ({ component }) => {
  ...
  return (
    <>
      <div
        key={symbol}
        className='relative rounded-full border border-gray-600 bg-transparent px-6 py-2 shadow-sm flex items-center space-x-3 hover:border-gray-400 focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-indigo-500'
      >
      ...
      <StyledTableData>
        {isNaN(Number(percentOfSet)) ? '0' : percentOfSet}%
      </StyledTableData>
...
const IndexComponentsTable = styled.div`
  display: grid;
  grid-template-columns: [logo] 25px repeat(4, 1fr);
