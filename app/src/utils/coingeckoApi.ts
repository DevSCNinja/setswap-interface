...
export const fetchHistoricalTokenMarketDataOnOptimism = (
  optimismTokenAddress?: string
) => {
  const coingeckoTokenIdentifier = `${OPTIMISM_CHAIN_DATA.coingeckoId}/contract/${optimismTokenAddress}`

  return fetchHistoricalTokenMarketData(coingeckoTokenIdentifier)
}

export const fetchHistoricalTokenMarketDataOnArbitrum = (
  arbitrumTokenAddress?: string
) => {
  const coingeckoTokenIdentifier = `${ARBITRUM_CHAIN_DATA.coingeckoId}/contract/${arbitrumTokenAddress}`
...
const getAssetPlatform = (chainId: number) => {
  ...
  if (chainId === ARBITRUM_CHAIN_DATA.chainId) return 'arbitrum-one'
  if (chainId === OPTIMISM_CHAIN_DATA.chainId) return 'optimistic-ethereum'
  return 'ethereum'
}
